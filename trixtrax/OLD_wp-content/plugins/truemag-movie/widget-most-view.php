...
class TM_Widget_Most_Viewed_Posts  extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'tm_widget_most_viewed_entries', 'description' => __( 'The most viewed posts on your site', 'bawpvc' ) );
		parent::__construct('most-viewed-posts', __( 'Most Viewed Posts', 'bawpvc' ), $widget_ops);
		$this->alt_option_name = 'tm_widget_most_viewed_entries';
	...
	function widget($args, $instance) {
		$cache = wp_cache_get('tm_widget_most_viewed_entries', 'widget');
		if ( !is_array($cache) )
			$cache = array();
		...
		<?php
			...
			$count = str_replace(')','',$count);
		?>
		<li><a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>"><?php if ( get_the_title() ) the_title(); else the_ID(); ?></a> <p><?php echo $count.' '. __('Views','bawpvc'); ?></p></li>
		<?php endwhile; ?>
		</ul>
		...
		wp_cache_set('tm_widget_most_viewed_entries', $cache, 'widget');
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['tm_widget_most_viewed_entries']) )
			delete_option('tm_widget_most_viewed_entries');

		return $instance;
	}

	function flush_widget_cache() {
		wp_cache_delete('tm_widget_most_viewed_entries', 'widget');
	}
