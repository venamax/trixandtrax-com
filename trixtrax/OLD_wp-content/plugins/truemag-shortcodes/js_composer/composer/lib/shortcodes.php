...
if (!class_exists('WPBakeryShortCode')) {
    abstract class WPBakeryShortCode extends WPBakeryVisualComposerAbstract {
        ...
        public function addInlineAnchors($content) {
            return ($this->isInline() || $this->isEditor() && $this->settings('is_container') === true ? '<span class="vc-container-anchor"></span>' : '').$content;
        }
        ...
        public function printIconStyles() {
            ...
            <style>
                ...
                .wpb-content-layouts .wpb-layout-element-button[data-element='" . esc_attr( $this->settings['base'] ) . "'] .vc-element-icon {
                    background-image: url(" . esc_url( $this->settings['icon'] ) . ");
        ...
        protected function loadTemplate( $atts, $content = null) {
            $output = '';
            if(!is_null($content)) $content = apply_filters('vc_shortcode_content_filter', $content);
        ...
        public function output($atts, $content = null, $base = '') {
            ...
            if($this->isInline() && method_exists($this, 'contentInline')) {
...
if (!class_exists('WPBakeryShortCode_UniversalAdmin')) {
    abstract class WPBakeryShortCode_UniversalAdmin extends WPBakeryShortCode {
        ...
        public function contentAdmin($atts,  $content) {
            ...
            if ( isset($this->settings['params']) ) {
                ...
                extract(shortcode_atts(
                    $shortcode_attributes
                    , $atts));
                $editor_css_classes = apply_filters('vc_edit_form_class', array('vc_span12a', 'wpb_edit_form_elements'));
        ...
        protected function singleParamEditForm($param, $param_value) {
			...
			//Checked- category by cactusthemes
            else if ( $param['type'] == 'categories' ) {
				$category_ids = get_all_category_ids();
				  //$current_value = explode(",", $param_value);
                //$values = is_array($param['value']) ? $param['value'] : array();
				$param_line .= '<div class="cat_cactus">';
				$slug_cat = '';
                foreach($category_ids as $val) {
                    $checked = '';
					$slug_cat = get_category($val);
                    if ( in_array($slug_cat->slug, explode(",", $param_value)) ) $checked = ' checked="checked"';
                    $param_line .= '<div class="item"><input class="'.$val.'"  value="'.$slug_cat->slug.'" type="checkbox" name="'.$param['param_name'].'"'.$checked.'>'.get_cat_name($val).'</div>';
                }
            }
