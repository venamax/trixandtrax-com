...
if(get_option('cnb') && !is_admin()) {
	...
	if($enabled == '1') {
		...
		function cnb_head() {
			...
			echo $credits."<style>#callnowbutton {display:none;} @media screen and (max-width:650px){#callnowbutton {display:block; ".$ButtonAppearance." height:70px; position:fixed; bottom:0px; border-top:2px solid ".darkerColor($options['color'])."; border-right:2px solid ".darkerColor($options['color'])."; border-left:2px solid ".darkerColor($options['color'])."; border-bottom:2px solid ".darkerColor($options['color'])."; background:url(" .plugins_url( 'text.png' , __FILE__ ). ") center 0px no-repeat ".$options['color']."; text-decoration:none; -webkit-box-shadow:0 0 5px #888; z-index:9999;}".$ButtonExtra."}</style>\n";
		}
		...
		function cnb_footer() {
			...
			if($limited) {
				if(is_single($show) || is_page($show)) {
					echo '<a href="http://www.trixandtrax.com/flips-fest-2015/" id="callnowbutton" '.$tracking.'>&nbsp;</a>';
				}
			} else {
				echo '<a href="http://www.trixandtrax.com/flips-fest-2015/" id="callnowbutton" '.$tracking.'>&nbsp;</a>';
			}
