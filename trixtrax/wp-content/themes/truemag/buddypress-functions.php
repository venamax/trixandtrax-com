...
if (function_exists('wpfp_link')) {
	add_action( 'bp_setup_nav', 'tm_setup_nav' );
	add_action( 'bp_setup_nav', 'tm_setup_subnav', 302 );
}
function tm_setup_nav() {
	  global $bp;
      bp_core_new_nav_item( array( 
            'name' => __( '<i class="fa fa-play"></i> Playlist', 'cactusthemes' ), 
            'slug' => 'playlist', 
            'position' => 150,
            'screen_function' => 'tm_playlist_screen',
			'default_subnav_slug' => 'myplaylist',
      ) );
}

function tm_setup_subnav() {
	global $bp;
	bp_core_new_subnav_item( array( 
		'name' => __( 'My Playlist','cactusthemes'),
		'slug' => 'myplaylist', 
		//'parent_url' => $bp->loggedin_user->domain . $bp->bp_nav['playlist']['slug'] . '/' ,
		'parent_url' => trailingslashit( bp_displayed_user_domain() . 'playlist' ) ,
...
function tm_playlist_screen(){	
	add_action( 'bp_template_content', 'tm_playlist_template' );
...
function tm_playlist_template() {
    ...
    if ($favorite_post_ids) {
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class('video-item'.(has_post_thumbnail()?'':' no-thumbnail')) ?>>
            ...
            <?php if(bp_displayed_user_id()==get_current_user_id()){ wpfp_remove_favorite_link(get_the_ID()); } ?>
