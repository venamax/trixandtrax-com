...
if(!function_exists('alter_comment_form_fields')){
	function alter_comment_form_fields($fields){
		...
		$fields['author'] = '<div class="cm-form-info"><div class="row comment-author-field collapse"><div class="col-md-4"><p class="comment-form-author"><input id="author" name="author" type="text" placeholder="'.($req ? '' : '').__('Your Name *','cactusthemes').'" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p></div>';
...
function ct_on_admin_init(){
	...
	if(isset($_POST['ct_update_theme'])){
		...
		if(ot_get_option('envato_auto_update',0) == 1){
...
function ct_admin_notice_theme_updated() {
    ?>
    <div class="updated">
        <p><?php _e( 'Theme Updated!', 'cactusthemes' ); ?></p>
    </div>
    <?php
}
...
?>
	...
	<meta property="og:type" content="<?php echo (get_post_format($post->ID) == 'video'? 'video.movie':'');?>"/>
